...
with DAG(
    ...
    # Читаем датасет, считаем среднюю стоимость и сохраняем в .csv
    mean_fares_dataset = PythonOperator(
        task_id='mean_fares_dataset',
        python_callable=mean_fare_per_class,
        dag=dag,
    )
    # Последния таска с инфой об окончании расчёта
    ...
    first_task >> create_titanic_dataset >> (pivot_titanic_dataset, mean_fares_dataset) >> last_task
